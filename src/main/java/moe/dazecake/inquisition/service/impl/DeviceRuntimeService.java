...
public class DeviceRuntimeService {
    ...
    public boolean recordHeartbeat(String deviceToken, Integer status, String assignmentId,
        ...
        synchronized (deviceToken.intern()) {
            ...
            // Any received heartbeat proves the device-to-server transport is alive. The legacy
            // status value remains available to old UI/scheduling code, while only 30 minutes
            // without a heartbeat changes the authoritative runtime state to OFFLINE.
